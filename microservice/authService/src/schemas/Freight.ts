...
FreightSchema.pre<FreightI>('save', function (next: NextFunction): void {
  this.vehicles.forEach((v: Vehicle, i): void => {
    const prices = getPricesVehicle(v.typeVehicle)
    this.vehicles[i].baseRate = prices.baseRate
    this.vehicles[i].pricePerKm = prices.pricePerKm
    this.vehicles[i].pricePerMin = prices.pricePerMin
  })
...
// @ts-ignore
// FreightSchema.pre<FreightI>('deleteOne', { document: true }, async function (next): Promise<void> {
//   console.log('REMOVENDOOOO')

//   // const idRemoved = this.getQuery()['_id']
//   // console.log(this.getQuery()['_id'])
//   // User.freights.pull({ _id: idRemoved })
//   // console.log(id)
